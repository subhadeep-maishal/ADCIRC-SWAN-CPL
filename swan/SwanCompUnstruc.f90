...
!$  integer, dimension(:), allocatable    :: tlist     ! vertex list for calling thread
    ...
    real(SZ), dimension(:), allocatable   :: temp      ! temporary array to store high precision data for UPDATER
...
!TIMG!PUN          call SWTSTA(213)
          do id = 1, MDC
             do is = 1, MSC
                temp(:) = real(ac2(id,is,:),SZ)
                call UPDATER(temp,dum1,dum2,1)
                ac2(id,is,:) = real(temp(:),4)
             enddo
          enddo
...
!TIMG!PUN          call SWTSTA(213)
          do j = 1, MCMVAR
             temp(:) = real(compda(:,j),SZ)
             call UPDATER(temp,dum1,dum2,1)
             compda(:,j) = real(temp(:),4)
          enddo
!TIMG!PUN          call SWTSTO(213)
       endif
       !
