...
!NCF          IF (MNPROC>1.AND.(RTYPE.EQ.'TABC')) THEN
...
!         subdomain                                                       40.31
     ...
     &           IY.GE.IYB .AND. IY.LE.IYE ) IONOD(IP) = INODE            40.31
      ...
      END IF                                                              40.31
!
      IF (IRQ.EQ.-999) GOTO 900
!
      IF (MNPROC.EQ.1) GOTO 900
!
      NOWNV = 0
      DO IP = 1, MIP
      ...
      IF (.NOT.LCOMPGRD) THEN
         ...
         DO IP = 1, MIP
            ...
            ELSE
               IVERTP = -1
            ENDIF
            IF ( IONOD(IP).EQ.MYPROC ) WRITE(NREF) IP, IVERTP
         ENDDO
         IF (IRQ.EQ.NREOQ) CLOSE(NREF)
      ENDIF
!
