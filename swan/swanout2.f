...
!NCF!
      ...
      ENDIF
      DO 70 IP = 1, MIP
!NPUN       IF ( .NOT.PARLL .OR. IONOD(IP).EQ.INODE ) THEN                     40.51
       IF ( MNPROC.EQ.1 .OR. IONOD(IP).EQ.MYPROC ) THEN                   41.36
        LINKAR = 1
        OUTLIN = '    '
        IF (RTYPE.EQ.'TABI') THEN
...
!       write keyword SWAN and version number
 ...
 103      FORMAT (I6, T41, A)                                             40.00
     ...
     &                    'locations in spherical coordinates'            33.09 nb!
          CRFORM = '(2F12.6)'                                             40.13
        ENDIF                                                             33.09 NB!
        IF ( MNPROC.EQ.1 .OR. .NOT.LCOMPGRD ) THEN
        WRITE (NREF, 103) MIP, 'number of locations'
        DO 110 IP = 1, MIP
...
!NPUN          IF ( .NOT.PARLL .OR. IONOD(IP).EQ.INODE )                       40.51
          IF ( MNPROC.EQ.1 .OR. IONOD(IP).EQ.MYPROC )                     41.36
     &       WRITE (NREF, 220) 'NODATA'                                   40.00
 220      FORMAT (A6)
          GOTO 290
        ENDIF
!NPUN        IF ( PARLL .AND. IONOD(IP).NE.INODE ) GOTO 290                    40.51
        IF ( MNPROC.GT.1 .AND. IONOD(IP).NE.MYPROC ) GOTO 290             41.36
        IF (ICUR.GT.0) THEN
          UX = VOQ(IP,VOQR(5))
          UY = VOQ(IP,VOQR(5)+1)
        ELSE
...
!           write 2d spectrum
          ...
          ELSE
!           write 1d spectrum
!NPUN            WRITE (NREF, 115) IP
            IF ( MNPROC.GT.1 .AND. LCOMPGRD ) THEN
