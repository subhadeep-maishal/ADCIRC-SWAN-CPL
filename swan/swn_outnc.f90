...
!
    ...
    subroutine swn_outnc_colspc ( RTYPE, OQI, OQR, MIP, KGRPGL )
      ...
      USE OCPCOMM2, ONLY: LENFNM, DIRCH2
      USE SIZES, ONLY: GLOBALDIR, LOCALDIR
!
!
!   --|-----------------------------------------------------------|--
!     | BMT ARGOSS                                                |
...
!
        ...
        basefile = basefile(ilpos:LENFNM)
        ilpos   = index(LOCALDIR, '0')-1

        do iproc = 1, NPROC
            binnr = HIOPEN+NREOQ+(NREF-HIOPEN-1)*NPROC+iproc

              write(binfile, '(A, I4.4, A, A)') LOCALDIR(1:ilpos), iproc - 1, DIRCH2, trim(basefile)
...
!
        ...
        if (ilpos.ne.0) ncfile = ncfile(1:ilpos-1)
         ilpos   = len(trim(LOCALDIR))+2
         ncfile = ncfile(ilpos:LENFNM)
         ncfile = trim(GLOBALDIR)//DIRCH2//trim(ncfile)
